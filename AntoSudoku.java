...
public class AntoSudoku implements Serializable {
	
	private AntoSudoku(int boardLen) {
		size=boardLen;
		len=(int)Math.sqrt(size);
		
		board=new ThreeeX3[len][len];		
		for (int i=0;i<len;i++)
			for (int j=0;j<len;j++)
				board[i][j]=new ThreeeX3(i*len+j, len);
	}
	...
	public static AntoSudoku readBoard(File file) throws Exception {
		FileInputStream fis=new FileInputStream(file);
		BufferedReader reader=new BufferedReader(new InputStreamReader(fis));
		String s=reader.readLine();
		String[]t=tokenize(s, "\t ", false);
		AntoSudoku su=new AntoSudoku(t.length);
		int row=0;
		do {
			int col=0;
			for (String ss:t) {
	...
	public static String[] tokenize(String s, String delim, boolean keepBlank) {
	 ...
	 ThreeeX3 getX3(int row, int col) {  // 0<=row<=size, 0<=col<=size
	...
	private void fillBoard() {
	 ...
	 int conflictRow(int r, int c, ThreeeX3 x3) { //conflict at row r in column
		int n=x3.get(r,c);
		int row=r+len*(x3.block()/len);
		for (int i=0;i<size;i++) {
			ThreeeX3 nx3=getX3(row, i);
			if (x3!=nx3) {
				int nn=getVal(row, i);
				if (nn==n) {
					return i;
	 ...
	 int conflictCol(int r, int c, ThreeeX3 x3) { //conflict at col c in row
		int n=x3.get(r,c);
		int col=c+len*(x3.block()%len);
		for (int i=0;i<size;i++) {
			ThreeeX3 nx3=getX3(i, col);
			if (x3!=nx3) {
				int nn=getVal(i, col);
				if (nn==n) {
					return i;
	 ...
	 boolean swapSelf(int r, int c, ThreeeX3 x3) {
		for (int i=0;i<len;i++) {
			for (int j=0;j<len;j++) {
	...
	boolean swap(int r, int c, ThreeeX3 x3) {
		boolean b, chg=false;
...
class ThreeeX3 implements Serializable {
	int len;
	private int block=0;
	private int[][] X3;
	private int[][] userSet;
	...
	void printBoard() {
		AntoSudoku.p("Block=%d%n", block);
		for (int r=0;r<len;r++)	{
			for (int c=0;c<len;c++) {
				if (isUserSet(r,c))
					AntoSudoku.p("{%d,%d}\t", X3[r][c], userSet[r][c]);
				else
					AntoSudoku.p("%d\t", X3[r][c]);
			}
			AntoSudoku.nl();
		}
		AntoSudoku.nl();
...
class MainAntoSudoku {
	
	static AntoSudoku su;
	
	public static final void main(String[] args) throws Exception {
		su=AntoSudoku.readBoard(new File(args[0]));
		su.printBoard();
		//shuffleBoard();
